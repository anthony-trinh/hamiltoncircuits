...
class Graph:
    ...
    def getHamiltonian(self):
        ...
        if (self.totalVertices() < 3):
            return None
        elif (self.tryVisiting(1, Hamiltonian, False)):
            print(Hamiltonian)
            return Hamiltonian
        else:
            return None

    def tryVisiting(self, totalvisited, Hamiltonian, flag):
        """
        ...
        if totalvisited == self.totalVertices():
            # check if you can connect the last and first vertex
            if ((self.getMatrix())[Hamiltonian.getVertex(Hamiltonian.totalVertices() - 1)][0] == 1):
                Hamiltonian.addVertex(0)
                return True
            else:
                return False

        for newVertex in range(1, self.totalVertices(), 1):
    ...
    def canConnect(self, vertex, totalvisited, Hamiltonian):
        # check if you can add vertex into
        # hamiltonian

        matrix = self.getMatrix()
        walk = Hamiltonian.getVertices()

        if matrix[vertex][walk[totalvisited - 1]] == 0:
            # check if there is an edge
            return False

        if (vertex in walk) and (vertex != 0):
            # check if the vertex is in there already
            return False
